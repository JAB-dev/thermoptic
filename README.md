...
> _"I don't believe it, thermoptic camouflage!"_
...
- Supports both **x86_64 (amd64)** and **ARM64 (arm64)** architectures. On x86_64, Google Chrome is used; on ARM64, Brave Browser is used instead.
...
- `thermoptic` analyzes the request to best determine what type of browser request it's _supposed_ to be (e.g. manual URL visit? Form submission? A `fetch()` request?).
...
## Why _this_ approach vs other solutions?
...
- Requiring great care to match the behavior of the "real" browser implementation perfectly. As a result, _any_ quirks or discrepancies can be used to differentiate these clients from the "real" browser implementation.
...
- Each layer of the stack such as TCP, TLS, HTTP are indistinguishable from the real browser because the request is made _using_ a real browser in the way it would _normally_ occur.
...
_NOTE: these cookies will stick around after the request as well. If you'd like to implement cleanup logic for cookies, please write a `thermoptic` hook._
...
### The Chrome Debugging Protocol _itself_ is fingerprintable!
...
### _Why_ release this tool? Don't you care about the evil actors you're enabling?!
...
Note that at this time `thermoptic` is only meant to be used with HTTP clients you explicitly trust. It's _not_ meant to be exposed to untrusted users.
